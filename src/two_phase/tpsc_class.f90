...
module tpsc_class
   ...
   ! List of know available bcond for this solver
   ...
   integer, parameter, public :: quick=1             !< Quick scheme  !!!!! Need to update to be consistent with VF transport
   ...
   type :: tpsc
      ...
      ! Density and diffusivity is still a field due to LES modeling
      real(WP) :: rho_l, rho_g                            !< This is our fluid densities
      real(WP), dimension(:,:,:), allocatable :: rho      !< This is our fluid densities (varies with phase)
      real(WP), dimension(:,:,:), allocatable :: rho_old  !< This is our fluid densities (varies with phase)
      real(WP), dimension(:,:,:), allocatable :: diff     !< These is our diffusivity (varies with phase and+SGS dynamic diffusivity)
      ...
      ! Scalar fluxes
      real(WP), dimension(:,:,:), allocatable :: FX,FY,FZ !< Scalar fluxes
      ...
      real(WP), dimension(:,:,:,:), allocatable :: hyb_x,hyb_y,hyb_z            !< Hybrid interpolation for U
   ...
   contains
      procedure :: print=>tpsc_print                    !< Output solver to the screen
      procedure :: setup                                  !< Finish configuring the scalar solver
      procedure :: add_bcond                              !< Add a boundary condition
      procedure :: get_bcond                              !< Get a boundary condition
      procedure :: apply_bcond                            !< Apply all boundary conditions
      procedure :: prepare_advection_upwind               !< Prepare for transport
      ...
      procedure :: get_olddensity                         !< Cacluate density
   end type tpsc

   !> Declare two-phase scalar solver constructor
   interface tpsc
...
contains
   ...
   !> Default constructor for two-phase scalar solver
   function constructor(cfg,scheme,name) result(self)
      ...
      allocate(self%rho_old(self%cfg%imino_:self%cfg%imaxo_,self%cfg%jmino_:self%cfg%jmaxo_,self%cfg%kmino_:self%cfg%kmaxo_)); self%rho  =0.0_WP
      ...
      select case (self%scheme)
       case (quick)
         ...
         if (self%cfg%no.lt.2) call die('[tpsc constructor] Two-phase scalar transport scheme requires larger overlap')
       ...
       case default
         call die('[tpsc constructor] Unknown two-phase scalar transport scheme selected')
      end select

      ! Create implicit two-phase scalar solver object
      self%implicit=ils(cfg=self%cfg,name='tpsc',nst=1+6*abs(self%stp1))
   ...
   subroutine add_bcond(this,name,type,locator,dir)
      ...
      if (present(dir)) then
         select case (lowercase(dir))
          ...
          case default; call die('[tpsc add_bcond] Unknown bcond direction')
         end select
      else
         if (new_bc%type.eq.neumann) call die('[tpsc apply_bcond] Neumann requires a direction')
         new_bc%dir=0
      end if
      ...
      select case (new_bc%type)
       ...
       case default
         call die('[tpsc apply_bcond] Unknown bcond type')
      end select
   ...
   subroutine get_bcond(this,name,my_bc)
      ...
      if (.not.associated(my_bc)) call die('[tpsc get_bcond] Boundary condition was not found')
   ...
   !> This routine upwind-transports densities, and prepares
   !> a modified velocity interpolation scheme for calculating scalar fluxes
   subroutine prepare_advection_upwind(this,fs,dt)
      use tpns_class, only : tpns
      implicit none
      class(tpsc), intent(inout) :: this   !< The two-phase scalar
      class(tpns), intent(in) :: fs        !< The two-phase flow solver
      ...
      this%hyb_x=this%itp_x; this%hyb_y=this%itp_y; this%hyb_z=this%itp_z
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               ! Fluxes on x-face
               if (abs(this%rho_old(i,j,k)-this%rho_old(i-1,j,k)).gt.rhoeps) then
                  vel=fs%U(i,j,k)
                  if (vel.ge.0.0_WP) then
                     this%hyb_x(:,i,j,k)=[1.0_WP,0.0_WP]
                  else
                     this%hyb_x(:,i,j,k)=[0.0_WP,1.0_WP]
                  end if
               end if
               ! Fluxes on y-face
               if (abs(this%rho_old(i,j,k)-this%rho_old(i,j-1,k)).gt.rhoeps) then
                  vel=fs%V(i,j,k)
                  if (vel.ge.0.0_WP) then
                     this%hyb_y(:,i,j,k)=[1.0_WP,0.0_WP]
                  else
                     this%hyb_y(:,i,j,k)=[0.0_WP,1.0_WP]
                  end if
               end if
               ! Fluxes on z-face
               if (abs(this%rho_old(i,j,k)-this%rho_old(i,j,k-1)).gt.rhoeps) then
                  vel=fs%W(i,j,k)
                  if (vel.ge.0.0_WP) then
                     this%hyb_z(:,i,j,k)=[1.0_WP,0.0_WP]
                  else
                     this%hyb_z(:,i,j,k)=[0.0_WP,1.0_WP]
                  end if
               end if
            end do
         end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_+1
         do j=this%cfg%jmin_,this%cfg%jmax_+1
            do i=this%cfg%imin_,this%cfg%imax_+1
               this%FX(i,j,k)=sum(this%hyb_x(:,i,j,k)*this%rho_old(i-1:i,j,k))*fs%U(i,j,k) ! Fluxes on x-face
               this%FY(i,j,k)=sum(this%hyb_y(:,i,j,k)*this%rho_old(i,j-1:j,k))*fs%V(i,j,k) ! Fluxes on y-face
               this%FZ(i,j,k)=sum(this%hyb_z(:,i,j,k)*this%rho_old(i,j,k-1:k))*fs%W(i,j,k) ! Fluxes on z-face
            end do
         end do
      end do
      ...
      do k=this%cfg%kmin_,this%cfg%kmax_
         do j=this%cfg%jmin_,this%cfg%jmax_
            do i=this%cfg%imin_,this%cfg%imax_
               this%rho(i,j,k)=this%rho_old(i,j,k)-dt* &
                  (sum(this%divsc_x(:,i,j,k)*this%FX(i:i+1,j,k)) &
                  +sum(this%divsc_y(:,i,j,k)*this%FY(i,j:j+1,k)) &
                  +sum(this%divsc_z(:,i,j,k)*this%FZ(i,j,k:k+1)))
            end do
         end do
      end do
   ...
   !> Solve for implicit two-phase tpsc residual
   ...
   subroutine get_olddensity(this,vf)
      ...
      print *,'get_olddensity'
      do k=this%cfg%kmino_,this%cfg%kmaxo_
         do j=this%cfg%jmino_,this%cfg%jmaxo_
            do i=this%cfg%imino_,this%cfg%imaxo_
               liq_vol=sum(vf%Lvol(:,:,:,i,j,k))
               gas_vol=sum(vf%Gvol(:,:,:,i,j,k))
               print *,'get_olddensity 1'
               tot_vol=gas_vol+liq_vol
               this%rho_old(i,j,k)=1.0_WP
               print *,'get_olddensity 2'
               if (tot_vol.gt.0.0_WP) this%rho_old(i,j,k)=(liq_vol*this%rho_l+gas_vol*this%rho_g)/tot_vol
            end do
         end do
      end do
      print *,'get_olddensity 3'
      ! Synchronize boundaries - not really needed...
      call this%cfg%sync(this%rho_old)
   end subroutine get_olddensity


   !> Print out info for two-phase scalar solver
   subroutine tpsc_print(this)
      use, intrinsic :: iso_fortran_env, only: output_unit
      implicit none
      class(tpsc), intent(in) :: this

      ! Output
      if (this%cfg%amRoot) then
         write(output_unit,'("Two-phase scalar solver [",a,"] for config [",a,"]")') trim(this%name),trim(this%cfg%name)
      end if

   end subroutine tpsc_print


end module tpsc_class
